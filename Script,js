...
// ============================================================
var EXCLUDE_PATTERN = /(拒绝|直连|群|邀请|返利|循环|官网|客服|网站|网址|获取|订阅|流量|到期|机场|下次|版本|官址|备用|过期|已用|联系|邮箱|工单|贩卖|通知|倒卖|防止|国内|地址|频道|无法|说明|使用|提示|特别|访问|支持|教程|关注|更新|作者|加入|超时|收藏|福利|好友|失联|选择|剩余|公益|发布|通路|登录|禁止|定时|渠道|牢记|永久|余额|阁下|本站|刷新|导航|⚠️|@|https?:\/\/|\.com\b|com\/|USE|USED|TOTAL|EXPIRE|EMAIL|Panel|Channel|Author|traffic)/i;

// 节点名称首部国旗 Emoji 提取
var FLAG_EMOJI_PATTERN = /^((?:\uD83C[\uDDE6-\uDDFF]){2})\s*/;

// 自然排序：匹配连续数字段
var DIGITS_PATTERN = /\d+/g;
var NATURAL_SORT_PAD_WIDTH = 16;

// ============================================================
// GitHub 代理（国内访问加速）
// ============================================================
var GITHUB_RAW_HOST    = "raw.githubusercontent.com";
var GITHUB_PROXY_PREFIX = "https://v6.gh-proxy.org/";

// ============================================================
// 核心策略组名称常量
// ============================================================
var POLICY_MAIN   = "节点选择";
var POLICY_DIRECT = "DIRECT";
var NAME_AUTO     = "自动选择";
var NAME_BALANCE  = "负载均衡";

var GROUP_NAME = Object.freeze({
  all:    "全部节点",
  other:  "其他节点",
  global: "GLOBAL",
});
...
// 图标资源
// ============================================================
var ICON_BASE_URL = "https://fastly.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/";
...
// DNS 服务器分组
// ============================================================
var DNS_DEFAULT = Object.freeze(["223.5.5.5", "119.29.29.29"]);
var DNS_CN = Object.freeze([
  "https://dns.alidns.com/dns-query#DIRECT",
  "https://doh.pub/dns-query#DIRECT",
]);
var DNS_GLOBAL = Object.freeze([
...
var OTHER_NODE_EXCLUDE_FILTER = "(?i)" + REGIONS.map(function (region) {
...
// 代理组基础配置模板
// ============================================================
var HEALTH_CHECK_BASE = Object.freeze({
...
var SELECT_BASE  = Object.freeze({ type: "select" });
var URLTEST_BASE = Object.freeze(Object.assign({}, HEALTH_CHECK_BASE, { type: "url-test",     tolerance: 100 }));
var BALANCE_BASE = Object.freeze(Object.assign({}, HEALTH_CHECK_BASE, { type: "load-balance", strategy: "sticky-sessions" }));

// ============================================================
// 规则集 URL 定义
// ============================================================
var RULESET_META_BASE   = "https://" + GITHUB_RAW_HOST + "/MetaCubeX/meta-rules-dat/meta/geo/";
var RULESET_DUSTIN_BASE = "https://" + GITHUB_RAW_HOST + "/DustinWin/ruleset_geodata/mihomo-ruleset/";

var RULESET_URL = Object.freeze({
  ...
  category_ntp:           RULESET_META_BASE   + "geosite/category-ntp.mrs",DownloadApps:           "https://" + GITHUB_RAW_HOST + "/AIsouler/MyClash/main/Rules/DownloadApps.txt",
...
var FAKEIP_FILTER_BASE = Object.freeze([
...
var POLICY_DNS_MAP = new Map([
  [POLICY_DIRECT, DNS_CN],
  [POLICY_MAIN,   DNS_GLOBAL],
]);
...
function getRegionIndex(nodeName) {
  ...
  for (var i = 0; i < REGIONS.length; i++) {
    var pattern = REGIONS[i].pattern;
    pattern.lastIndex = 0;
    var match = pattern.exec(nodeName);
...
function processProxies(rawProxies) {
  ...
  // 第一遍：过滤 + 构造排序条目
  for (var i = 0; i < rawProxies.length; i++) {
    var proxy = rawProxies[i];
    var name  = proxy && typeof proxy.name === "string" ? proxy.name.trim() : "";
    if (!name || seenNames.has(name) || EXCLUDE_PATTERN.test(name)) continue;
    seenNames.add(name);

    var flagMatch = FLAG_EMOJI_PATTERN.exec(name);
    sortEntries.push({
  ...
  var regionBuckets = REGIONS.map(function () { return []; });
  var otherBucket   = [];
  var proxies       = [];
  var proxyNames    = [];

  for (var j = 0; j < sortEntries.length; j++) {
    var entry = sortEntries[j];
...
function makeRuleProvider(rulesetName, behavior, format) {
  var rawUrl = RULESET_URL[rulesetName];
  if (!rawUrl) throw new Error("未找到规则集 URL: " + rulesetName);

  var isGithubUrl = rawUrl.indexOf(GITHUB_RAW_HOST) !== -1;
  var fileExt     = format === "text" ? "txt" : format;
  var provider = {
    type:     "http",
    format:   format,
    behavior: behavior,
    interval: 86400,
    url:      isGithubUrl ? GITHUB_PROXY_PREFIX + rawUrl : rawUrl,path:     "./ruleset/" + rulesetName + "." + fileExt,
  };
...
function buildRuleArtifacts() {
  var ruleProviders    = {};
  var rules            = [];
  var nameserverPolicy = { "+.arpa": "system" };
  var fakeIpSet        = new Set(FAKEIP_FILTER_BASE);

  for (var i = 0; i < RULE_DEFS.length; i++) {
    var ruleDef = RULE_DEFS[i];
    ...
    var rulesetName = ruleDef.name;
    var behavior    = ruleDef.behavior;
    var policy      = ruleDef.policy;
    var format      = ruleDef.format != null ? ruleDef.format : "mrs";
    ...
    if (behavior === "domain") {
      var rulesetRef = "rule-set:" + rulesetName;
      var nameserver = ruleDef.dns != null ? ruleDef.dns : (policy && POLICY_DNS_MAP.get(policy));
      if (nameserver) nameserverPolicy[rulesetRef] = nameserver;

      // fakeIp 未显式指定时，DIRECT 策略的域名默认加入白名单（走真实 DNS）
      var wantFakeIp = ruleDef.fakeIp != null ? ruleDef.fakeIp : (policy === POLICY_DIRECT);
...
var RULE_ARTIFACTS = buildRuleArtifacts();
...
function makeNodeGroup(baseConfig, name, icon, members, providerNames, filter, excludeFilter) {
  var group = Object.assign({}, baseConfig, { name: name });
...
function makeRegionContainerGroup(name, icon, members, providerNames, filter, excludeFilter, autoGroupName, balanceGroupName) {
  // 自动选择和负载均衡始终置顶
  var proxies = [autoGroupName, balanceGroupName];
  for (var i = 0; i < members.length; i++) proxies.push(members[i]);

  var group = Object.assign({}, SELECT_BASE, { name: name, icon: icon, proxies: proxies });
  attachProxyProviders(group, providerNames, filter, excludeFilter);
  return group;
}
...
function buildGroups(proxyData, providerNames) {
  var proxyNames        = proxyData.proxyNames;
  var regionBuckets     = proxyData.regionBuckets;
  var otherBucket       = proxyData.otherBucket;
  var hasProxyProviders = providerNames.length > 0;

  // --- 地区分组 ---
  var regionSelectNames = [];  // 用于主选择组的地区入口列表
  var allGroups         = []; // 收集所有代理组，最终一次性拼接

  for (var i = 0; i < REGIONS.length; i++) {
    var region  = REGIONS[i];
    var members = regionBuckets[i];
    if (!members.length && !hasProxyProviders) continue;

    // provider 模式下通过正则过滤节点，直连模式下已经分桶无需过滤
    var regionFilter     = hasProxyProviders ? "(?i)" + region.pattern.source : null;
    var autoGroupName    = region.name + "-" + NAME_AUTO;
    var balanceGroupName = region.name + "-" + NAME_BALANCE;

    var autoGroup    = makeNodeGroup(URLTEST_BASE, autoGroupName,    ICON.auto,    members, providerNames, regionFilter, null);
    var balanceGroup = makeNodeGroup(BALANCE_BASE, balanceGroupName, ICON.balance, members, providerNames, regionFilter, null);
  ...
  // --- 其他节点分组 ---
  var hasOtherNodes = otherBucket.length > 0 || hasProxyProviders;
  if (hasOtherNodes) {
    var otherExcludeFilter = hasProxyProviders ? OTHER_NODE_EXCLUDE_FILTER : null;
    var otherAutoName      = GROUP_NAME.other + "-" + NAME_AUTO;
    var otherBalanceName   = GROUP_NAME.other + "-" + NAME_BALANCE;

    var otherAutoGroup    = makeNodeGroup(URLTEST_BASE, otherAutoName,    ICON.auto,    otherBucket, providerNames, null, otherExcludeFilter);
    var otherBalanceGroup = makeNodeGroup(BALANCE_BASE, otherBalanceName, ICON.balance, otherBucket, providerNames, null, otherExcludeFilter);
  ...
  // --- 全局自动/负载均衡子组 ---
  var globalAutoGroup    = makeNodeGroup(URLTEST_BASE, NAME_AUTO,    ICON.auto,    proxyNames, providerNames, null, null);
  var globalBalanceGroup = makeNodeGroup(BALANCE_BASE, NAME_BALANCE, ICON.balance, proxyNames, providerNames, null, null);
  globalAutoGroup.hidden    = true;
  globalBalanceGroup.hidden = true;

  // --- 全部节点直选组 ---
  var allNodesGroup = makeNodeGroup(SELECT_BASE, GROUP_NAME.all, ICON.all, proxyNames, providerNames, null, null);

  // --- 主选择组：自动 > 负载均衡 > 全部 > 各地区 > 其他 ---
  var mainGroupProxies = [NAME_AUTO, NAME_BALANCE, GROUP_NAME.all].concat(regionSelectNames);
  if (hasOtherNodes) mainGroupProxies.push(GROUP_NAME.other);
  var mainGroup = Object.assign({}, SELECT_BASE, {
    name: POLICY_MAIN, icon: ICON.select, proxies: mainGroupProxies,
  });

  // --- 服务专属组：默认选主策略，也可切换到各地区 ---
  var serviceProxyOptions = [POLICY_MAIN].concat(regionSelectNames);
  var serviceGroups = [];
  ...
  var orderedGroups = [mainGroup, allNodesGroup]
  ...
  var allGroupNames = [];
  for (var g = 0; g < orderedGroups.length; g++) allGroupNames.push(orderedGroups[g].name);
  var globalGroup = Object.assign({}, SELECT_BASE, {
...
function validateInput(rawProxies, proxyProviders) {
  var hasProxies   = Array.isArray(rawProxies)    && rawProxies.length > 0;
  ...
  if (hasProxies && !hasProviders) {
    var allDummy = rawProxies.every(function (proxy) {
      var type = (proxy && typeof proxy.type === "string" ? proxy.type : "").toLowerCase();
...
function main(config) {
  var inputConfig = isPlainObject(config) ? config : {};

  // 透传原始配置中的 proxy-providers 等字段，排除会被覆写的 proxies 和指纹
  var passthroughConfig = {};
  for (var key in inputConfig) {
    if (!Object.prototype.hasOwnProperty.call(inputConfig, key)) continue;
    if (key === "global-client-fingerprint" || key === "proxies") continue;
    passthroughConfig[key] = inputConfig[key];
  }

  var rawProxies        = inputConfig.proxies;
  var proxyProviders    = passthroughConfig["proxy-providers"];
  var hasProxyProviders = validateInput(rawProxies, proxyProviders);

  var proxyData     = processProxies(Array.isArray(rawProxies) ? rawProxies : []);
  var providerNames = hasProxyProviders ? Object.keys(proxyProviders) : [];

  // BASE_CONFIG 覆盖透传配置，确保核心参数不被订阅原始值污染
  var outputConfig = Object.assign({}, passthroughConfig, BASE_CONFIG);
  outputConfig.proxies             = proxyData.proxies;
  outputConfig["proxy-groups"]     = buildGroups(proxyData, providerNames);
  outputConfig.rules               = RULE_ARTIFACTS.rules;
  outputConfig["rule-providers"]   = RULE_ARTIFACTS.ruleProviders;
  outputConfig.dns                 = buildDNS(RULE_ARTIFACTS.nameserverPolicy, RULE_ARTIFACTS.fakeIpFilter);
